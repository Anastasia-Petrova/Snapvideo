...
final class VideoEditorViewControllerTests: XCTestCase {
    func test_openLooks_and_closeLooks_setCorrect_topLooksConstraint_constant() {
        ...
        let vc = VideoEditorViewController(url: url, selectedFilterIndex: 0, filters: filters, tools: [])
    ...
    func test_openTools_and_closeTools_setCorrect_topToolsConstraint_constant() {
        ...
        let url = URL(fileURLWithPath: path)
        let vc = VideoEditorViewController(url: url, selectedFilterIndex: 0, filters: filters, tools: [.vignette(tool: VignetteTool())])
    ...
    func test_openExportMenu_and_closeExportMenu_setCorrect_topExportMenuConstraint_constant() {
        ...
        vc.openExportMenu()
        XCTAssertEqual(
            vc.topExportPanelConstraint.constant,
            vc.exportPanel.frame.height + vc.tabBar.frame.height
        )
